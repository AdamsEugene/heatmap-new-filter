...
function clearChanges() {
  ...
  for (let [elementId, changes] of allChanges) {
    ...
    // Remove added 'adams' from attributes
    element.getAttributeNames().forEach((attrName) => {
      const attrValue = element.getAttribute(attrName);
      if (attrValue.includes(" adams")) {
        element.setAttribute(attrName, attrValue.replace(" adams", ""));
      }
    });

    // Clear specific styles we've been tracking
    ["display", "opacity", "visibility", "transform"].forEach((prop) => {
      element.style.removeProperty(prop);
    });

    // Remove the ID if we added one for display rule
  ...
  console.log("All changes have been cleared.");
}
